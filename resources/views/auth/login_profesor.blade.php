    ...
    <div class="grid-container">
        ...
        <div class="grid-item grid-item--top bg-uabc-sec">
            ...
            <div class="w-100 d-flex justify-content-center align-items-center align-content-center flex-column">
                @include('svg.logo')
                <p class="m-0 mt-2 text-white title-software">SOFTWARE DE SIMULACION CENEVAL</p>
                <p class="m-0 mt-3 text-white version-user">PROFESOR</p>
            </div>
        </div>
        ...
        <div class="grid-item grid-item--body d-flex flex-wrap">
            ...
            <form class="w-100" method="POST" action="{{ route('profesor.login.submit') }}">
                @csrf
                <div class="mb-3">
                    <label for="matricula" class="form-label">Codigo de empleado</label>
                    {{-- <input type="number" class="form-control @error('email') is-invalid @enderror" value="{{ old('email') }}" id="matricula" name="email" required> --}}
                    <input type="number" class="form-control @error('email') is-invalid @enderror" value="{{ old('email') }}" id="matricula" name="email" required>
                    <div id="emailHelp" class="form-text">Ingrese # de empleado</div>
                    @error('email')
